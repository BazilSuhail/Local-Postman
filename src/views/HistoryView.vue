  ...
  <div>
    ...
    <div class="flex items-center justify-between">
      <div>
        <div class="flex items-center gap-3 mb-2">
          ...
          <h1
            class="text-[30px] font-bold bg-gradient-to-r from-emerald-600 to-emerald-800 bg-clip-text text-transparent">
            Request History
          </h1>
        </div>
        <p class="text-gray-600 ml-[52px] mt-[-8px] text-[15px]">Track and manage your API request history</p>
      </div>

      <!-- Stats -->
      <div v-if="history.length > 0" class=" bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
      <div class="flex flex-col sm:flex-row gap-4">
        ...
        <div class="relative flex-1">
          ...
          <input v-model="searchTerm" type="text" placeholder="Search requests..."
        ...
        <!-- Clear All Button -->
        <button @click="clearAllHistory" :disabled="history.length === 0"
    ...
    <div v-if="filteredHistory.length === 0"
      ...
      <p class="text-gray-500">
        {{ searchTerm || filterMethod ? 'Try adjusting your search or filter criteria' : 'Your request history will apear here' }}
      </p>
    </div>
      ...
      <li v-for="(item, index) in filteredHistory" :key="index"
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4 flex-1 min-w-0">
            <!-- Method Badge -->
            <div :class="`px-3 py-1 rounded-full text-[13px] font-semibold border ${getMethodColor(item.method)}`">
              {{ item.method }}
            </div>

            <!-- URL -->
            <div class="flex items-center gap-2 flex-1 min-w-0">
              <span class="text-gray-800 font-medium text-[17px] truncate">{{ item.url }}</span>
            </div>

            <!-- Auth/File Indicators -->
            <div class="flex items-center gap-2">
              <div v-if="item.hasAuth"
              ...
              <div v-if="item.hasFile"
                class="flex items-center gap-1 px-2 py-1 bg-purple-100 text-purple-700 rounded-full text-xs">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
